  ...
  <body style="margin: 0; padding: 0; background-color: #000000; text-align: center;">
    <div style="background-color: #000000; color: #ffffff; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 60px 20px; text-align: center; -webkit-font-smoothing: antialiased;">
      ...
      <div style="max-width: 500px; margin: 0 auto; border: 1px solid #27272a; border-radius: 24px; padding: 50px 40px; background-color: #09090b; box-shadow: 0 20px 40px rgba(0,0,0,0.8);">
        ...
        <a href="https://lumen.com/archive" style="display: inline-block; background-color: #ffffff; color: #000000; text-decoration: none; padding: 18px 36px; border-radius: 30px; font-size: 11px; font-weight: bold; letter-spacing: 3px; text-transform: uppercase; transition: opacity 0.3s;">
          Private Archive 입장하기
        </a>
...
export async function POST(req: Request) {
  try {
    ...
    // 수신자 이메일이 없는 경우 방어 로직 (테스트 환경 등)
    // 🍏 안전한 테스트를 위해 항상 cto@yeahplus.co.kr로 강제 수신되도록 세팅되어 있습니다.
    const targetEmail = "cto@yeahplus.co.kr"; 

    // 🚀 Resend 이메일 발송 실행
    const { data, error } = await resend.emails.send({
      from: 'LUMEN Concierge <onboarding@resend.dev>', // 정식 도메인 연결 전에는 resend.dev 사용
      to: targetEmail,
